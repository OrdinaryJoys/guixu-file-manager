...
测试启动前会在 `e2e/.artifacts` 一次性创建四文件隔离资料库，并把一个夹具设为 400 天未修改以验证只读清理建议；仅 `e2e` Cargo feature 会读取 `GUIXU_E2E_LIBRARY` 并注册该目录，普通与发布构建不存在此入口。当前 5 个场景覆盖：真实扫描、搜索与文本预览、智能文件夹、清理建议、整理计划预览、精确重复与相似文本后台报告、真实重命名/历史/安全撤销，以及设置即时生效与重新读取。截图和数据库均写入隔离目录，不会使用个人应用数据。
