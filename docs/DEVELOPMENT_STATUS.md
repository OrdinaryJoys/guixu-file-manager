...
- 侧栏提供只读“清理建议”：按大文件、长期未修改、临时文件和陈旧下载解释候选，只允许查看或在文件夹中显示，不执行自动清理；
...
- `cd e2e && npm test`：macOS 嵌入式 WebDriver 原生 App 5 项通过，覆盖真实资料库扫描、搜索/预览、智能文件夹、只读清理建议、整理计划、重复/相似报告、真实重命名/历史/撤销及设置；
- `git diff --check`：通过；
