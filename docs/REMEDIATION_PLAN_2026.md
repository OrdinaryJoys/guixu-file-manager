...
> 状态：实施跟踪稿 v4（2026-08-03）
>
> 依据：2026-07-29 全库审计、源码回读、SQLite 数据层实测，以及代码/Web/原生窗口交叉验证；2026-08-02 基础复测为 91 个 Rust、3 个前端竞态测试全绿
>
> 关联文档：[开发进度](./DEVELOPMENT_STATUS.md)、[测试方案](./TEST_STRATEGY_2026.md)、[整体方案](./MASTER_PLAN.md)、[算法方案](./ALGORITHM_DESIGN_2026.md)、[UI 方案](./UI_POLISH_PLAN.md)
>
...
| --- | --- | --- | --- |
| P0 数据安全 | 核心修复完成 | 不完整扫描不再误标缺失；复制撤销逐项复检；跨卷移动按原文件 ID 更新身份并清理缓存 | 真实跨卷、拔盘、权限/空间不足和强杀节点验证 |
| P1 状态机 | 核心完成 | 条件状态转移、启动消费 queued、活动任务去重、结果与完成状态同事务、worker 打开失败收敛到 failed；recovery_needed 恢复撤销闭环（仅回滚已发布条目，失败条目与占位目标原样保留） | 完整强退矩阵与真实跨卷实机 |
| P2 性能 | 部分验收 | FTS 更新触发器 guard；重复/相似分析使用 worker 连接与缓存；MIH 已建立 10k/100k Criterion 与 1M 一次性规模基线 | 10 万真实文件端到端与冷图片 p95 仍待建立 |
| P3 前端竞态 | 完成并自动化首批 | `LatestRequestGate`、切库失效、列表/搜索/预览防旧响应、搜索态保留、计划互斥和通知/键盘修复；概览与智能文件夹已加请求门（任务控制为全局状态且已有重入守卫） | 真实慢速存储上的切库竞态抽测 |
...
本轮验证基线为 92 项 Rust 测试、7 项前端请求竞态测试、前端契约、严格 Clippy、JS 语法、diff whitespace、官方 Tauri debug `.app` 构建、8 场景嵌入式 WebDriver 原生资料库 E2E，以及 1180×760 / 900×620 历史真实窗口点击验证。完整测试分层和下一阶段矩阵见 [测试与分析方案](./TEST_STRATEGY_2026.md)，逐模块证据与未完成门禁见 [交叉验证矩阵](./CROSS_VALIDATION_2026.md)。附录 A 是 2026-07-29 的原始问题快照；其中条目即使已修复也保留原文以便追溯，当前状态只能以上表和 [开发进度](./DEVELOPMENT_STATUS.md) 为准。

---

## 1. 历史审计结论摘要

本节描述 2026-07-29 修复前的根因，不是当前未修复清单。

- **总计 68 项缺陷：高 8 / 中 21 / 低 39**（附录 A 为完整清单）。
- 算法核心（SimHash/MinHash、dHash/pHash、三段 MIH、RRF/凸融合、三级精确去重）已补齐单元与候选完整性验证；既有缺陷集中在**跨层集成**：索引对账、任务状态机、跨卷操作、FTS 同步、前端异步竞态。
- 最严重的是三类**数据安全**问题：撤销删除用户新内容（H2）、跨卷移动写坏索引（H3）、扫描跳过子树被误判为已删除（H1）。
- 当时的测试虽然全部通过，但下述缺陷落在测试盲区；已实施批次必须保留对应回归测试（见第 10 节）。
...
**实施结果**：已引入可单测的 `LatestRequestGate`，列表/搜索共享最新请求语义，预览使用独立请求门；切换资料库主动失效旧上下文，过期响应和旧错误均不再提交。首批 3 项 Node 单测覆盖最新请求、切库失效和乱序响应。
