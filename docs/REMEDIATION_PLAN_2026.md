...
| P3 前端竞态 | 完成并自动化首批 | `LatestRequestGate`、切库失效、列表/搜索/预览防旧响应、搜索态保留、计划互斥和通知/键盘修复 | 扩展智能文件夹、概览和任务控制的延迟/错误注入 |
