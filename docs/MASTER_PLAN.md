...
最新进度和阻塞项以 [开发进度](./DEVELOPMENT_STATUS.md) 为准；本文负责相对稳定的产品、架构和里程碑定义。所有文档职责见 [文档中心](./README.md)。

配套文档：

- [当前开发进度](./DEVELOPMENT_STATUS.md)
- [测试与分析方案](./TEST_STRATEGY_2026.md)
- [交叉验证矩阵](./CROSS_VALIDATION_2026.md)
...
| M1 后台任务闭环 | 核心完成 | 独立扫描连接、进度、暂停/恢复/取消、事件推送、多资料库监听 | 基础状态与进程级强杀（意图/发布节点）已过；休眠、拔盘与事件风暴待补 |
| M2 文件浏览交互 | 核心完成 | 排序、列表/网格、键盘导航、批量栏、预览、打开、访达中显示 | 基础 App E2E 已过；发布构建打开/显示与 Quick Look 待补 |
| M3 搜索与智能文件夹 | 已完成 | Unicode/Trigram 搜索、结构化条件、版本化智能文件夹规则 | 查询解析、迁移、短词与子串搜索测试 |
| M4 通用文件操作 | 核心完成 | 批量重命名、安全复制、通用移动与受控隔离区删除 | 基础事务/撤销与多文件原生 E2E（重命名/整理/废纸篓闭环）已过；真实跨卷、权限/空间注入待补 |
...
| M6 平台增强与发布 | 待开始 | Quick Look、Spotlight/PDF/Vision、Windows provider、安装发布 | 真机矩阵、签名/权限、升级/降级和安装包验收 |

截至 2026-08-06 基础复测，Rust workspace 共 **92 项测试通过、0 项失败**（含恢复撤销仅回滚已发布条目），前端请求竞态单测 **7 项通过**（含错误注入语义），原生 Tauri/WKWebView 场景 **8 项通过**（含多文件操作闭环与核心 API 注入回归断言），进程级强杀矩阵 **3/3 通过**。这证明现有核心路径满足当前测试约束，不等同于“核心完成”模块已经通过发布门，也不等同于 M5–M6 已完成。完整分层、数据集和发布门见 [测试与分析方案](./TEST_STRATEGY_2026.md)。
...
Tauri 主线数据库名为 `guixu.sqlite3`，默认位于系统分配的应用数据目录；开发或测试可通过 `GUIXU_DATA_DIR` 指定目录。当前 schema 版本为 **12**，迁移只允许前向、顺序执行，并拒绝打开高于当前程序支持版本的数据库。v7 增加发布校验信息，v8 增加本地应用设置，v9 增加绑定文件快照与算法版本的哈希缓存，v10 增加持久后台任务结果，v11 增加文件特征、相似运行与候选边，v12 为 FTS 同步触发器增加 WHEN 守卫（消除无变化重扫的 O(n²) 退化，数据模型不变）。
...
详细执行矩阵、环境、性能预算、故障注入和发布退出条件统一记录在 [测试与分析方案](./TEST_STRATEGY_2026.md)；本节保留总体层级。
...
说明：Tauri 官方 debug `.app` 已完成真实窗口、核心 capability、相似任务、设置页、900×620 响应式和 macOS 原生目录选择器验收；2026-08-03 实机检查发现并修复 capabilities 缺 `core:default` 导致生产启动无法调用核心 API 的阻断问题（E2E 因 wdio 插件自动补权掩盖），修复后真实窗口验证库恢复、列表、概览、任务中心正常。真实“打开/在访达显示”、正式签名、公证与安装包仍需发布阶段真机验收。
...
- [开发进度](./DEVELOPMENT_STATUS.md) 是当前状态、阻塞项和下一步的首要事实来源。
- 本文是总体范围、架构、里程碑定义和完成标准的事实来源。
- [测试与分析方案](./TEST_STRATEGY_2026.md) 定义测试矩阵与发布退出条件；[交叉验证矩阵](./CROSS_VALIDATION_2026.md) 保存实际证据。
...
M5.3 已完成。M5.4 已完成文本 SimHash + MinHash、受限图片解码、EXIF 方向归一化 dHash + DCT pHash、快照缓存、精确 MIH 候选、持久任务和 Web/App 同步只读界面；阈值评估器、precision 安全门、10k/100k 统计基线与 1M 一次性规模门禁也已建立。下一步采集真实标注集并校准门限，再接入 Vision 增强 provider。完成准确率评估前不开放批量清理。
