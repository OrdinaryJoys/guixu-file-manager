...
| M1 后台任务闭环 | 核心完成 | 独立扫描连接、进度、暂停/恢复/取消、事件推送、多资料库监听 | 基础状态测试已过；强杀、休眠、拔盘与事件风暴待补 |
...
| M4 通用文件操作 | 核心完成 | 批量重命名、安全复制、通用移动与受控隔离区删除 | 基础事务/撤销已过；真实跨卷、强退和多文件原生 E2E 待补 |
...
截至 2026-08-02 基础复测，Rust workspace 共 **91 项测试通过、0 项失败**，前端请求竞态单测 **3 项通过**，原生 Tauri/WKWebView 冒烟 **5 项通过**。这证明现有核心路径满足当前测试约束，不等同于“核心完成”模块已经通过发布门，也不等同于 M5–M6 已完成。完整分层、数据集和发布门见 [测试与分析方案](./TEST_STRATEGY_2026.md)。
