...
| M2 文件浏览交互 | 已完成 | 排序、列表/网格、键盘导航、批量栏、预览、打开、访达中显示 | 浏览器演示交互；命令规格与授权边界测试 |
| M3 搜索与智能文件夹 | 已完成 | Unicode/Trigram 搜索、结构化条件、版本化智能文件夹规则 | 查询解析、迁移、短词与子串搜索测试 |
| M4 通用文件操作 | 待实现 | 重命名、复制、移动、移入废纸篓及统一批处理体验 | 故障注入、跨卷、冲突、撤销、重启恢复 |
| M5 算法生产化 | 待实现 | 哈希缓存、稳定保留评分、相似图片/文本候选与解释 | 基准集、准确率、性能、内存和取消测试 |
| M6 平台增强与发布 | 待实现 | Quick Look、Spotlight/PDF/Vision、Windows provider、安装发布 | 真机矩阵、签名/权限、升级/降级和安装包验收 |

截至本文检查时，Rust workspace 共 **49 项测试通过、0 项失败**。这证明现有核心路径满足当前测试约束，不等同于 M4–M6 已完成。
...
- 可解释保留评分：用户固定项 > 路径偏好 > 元数据完整度 > 修改时间 > 稳定路径顺序；
...
主导航按“资料库、智能文件夹、重复文件、任务、历史/设置”组织。统一交互约束如下：
...
Tauri 主线数据库名为 `guixu.sqlite3`，默认位于系统分配的应用数据目录；开发或测试可通过 `GUIXU_DATA_DIR` 指定目录。当前 schema 版本为 **5**，迁移只允许前向、顺序执行，并拒绝打开高于当前程序支持版本的数据库。
...
1. 扩展领域模型：`Rename/Move/Copy/Trash` 意图、冲突策略和逐项结果。
...
| 浏览器交互测试 | 页面行为 | 选择、多选、快捷键、批量栏、预览、任务和错误态 |
...
说明：`type-complexity` 目前为一个已知的存储层签名复杂度债务，应在 M4 抽取操作仓储结构时消除。浏览器演示已覆盖当前页面交互，但安装后的 `.app` 中真实“打开/在访达显示”仍需在装好 Tauri CLI 并完成真机 E2E 后验收。
...
下一步从 **M4.1 通用操作模型与仓储接口** 开始。首个增量只实现批量重命名的计划生成与预检，不立即执行；验证通过后再接入执行和撤销。这样可以先稳定通用契约，再复用于复制、移动和废纸篓操作。
